...
export const useUserStore = defineStore("user", () => {
    ...
    // 【狀態 (State)】
    // 建立一個名為 userInformation 的 ref，初始值為 null；
    // 這就像在倉庫裡準備一個叫 "userInformation" 的空箱子，之後用來存放登入成功的會員資料；
    ...
    const login = async(getUsername, getEmail, getPassword) => {
        try{
            ...
            //如果登入成功把回傳的資料存到 userInformation 這個 ref 裡
    ...
    const logout = () => {
        //會使用null是因為userInformation一開始就定義裡面會放物件
        userInformation.value = null;  
        isLoggedIn.value = false;
    };
    ...
    const updatePassword = async (getNewPassword) => {
       ...
       if(!isLoggedIn.value){
        ...
        try{
            ...
            //使用樣板字串 `` 來動態嵌入 userId
            const response = await axios.put(`http://localhost:3000/api/user/${userId}`, pw);
            console.log(response.data);
            return true;
    ...
    const deleteAccount = async () => {
        ...
        try{
            ...
            //但因為執行了登出，會把userInformation.value變成null，導致
            logout();
            return true;
